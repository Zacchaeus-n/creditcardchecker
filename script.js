...
let findInvalidCards = (ar) => {
  invalidArr = ar.filter((invCard) => validateCred(invCard));
  return invalidArr;
};

findInvalidCards(batch);

let idInvalidCardCompanies = (cardCompany) => {
  let companyArr = [];
  //reverse arrays, iterate and test 1st digit
  for (let i = 0; i < cardCompany.length; i++) {
    let revCompany = cardCompany[i].reverse();
    if (revCompany[0] === 3) {
      companyArr[i] = "Amex (American Express)";
    } else if (revCompany[0] === 4) {
      companyArr[i] = "Visa";
    } else if (revCompany[0] === 5) {
      companyArr[i] = "Mastercard";
    } else if (revCompany[0] === 6) {
